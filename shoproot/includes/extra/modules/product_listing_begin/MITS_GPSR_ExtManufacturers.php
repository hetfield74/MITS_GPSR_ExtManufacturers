...
if (defined('MODULE_MITS_GPSREXTMANUFACTURERS_STATUS') && MODULE_MITS_GPSREXTMANUFACTURERS_STATUS == 'true') {
    if (isset($manufacturers_id) && isset($manufacturers_array[$manufacturers_id]) && basename($PHP_SELF) != FILENAME_ADVANCED_SEARCH_RESULT) {
        ...
        foreach ($valid_params as $param) {
            $module_smarty->assign(strtoupper($param), isset($manufacturer[$param]) && !empty($manufacturer[$param]) ? $manufacturer[$param] : '');
        }

        if (defined('MODULE_MITS_GPSREXTMANUFACTURERS_APPENDTO_LISTING_DESC') && MODULE_MITS_GPSREXTMANUFACTURERS_APPENDTO_LISTING_DESC == 'true') {
            ...
            if (isset($current_category_id) && $current_category_id != '0') {
                $module_smarty->assign('MANUFACTURER_DESCRIPTION', ($manufacturer['manufacturers_description'] ?? '') . $mits_gpsr_text);
            } else {
                $module_smarty->assign('CATEGORIES_DESCRIPTION', ($category['categories_description'] ?? '') . $mits_gpsr_text);
            }
        }
    }
}